...
class Tropa:
    def __init__(self, tipo, quantidade, nivel):
        self.set_tipo(tipo)
    ...
    def get_from_file(self):
        ...
        if(linhas == ''):
            ...
            for i in range(n): # Pega as tropas da linha e coloca na lista de objetos chamda de tropas
                aux_t, aux_q, aux_p = linhas[i].split('-')
                aux_p = aux_p.replace("\n", "")
                total += int(aux_q)
                tropas.append(Tropa(aux_t, int(aux_q), int(aux_p)))
        arquivo.close()
        return total
class Comandante:
...
for i in range(n):
    for j in range(qtd):
        if(j == 0):
            comandantes[j].tropas.append(Tropa(tropas[i].get_tipo(), (tropas[i].get_quantidade()//qtd + tropas[i].get_quantidade()%qtd ), tropas[i].get_nivel() ))
        else:
            comandantes[j].tropas.append(Tropa(tropas[i].get_tipo(), (tropas[i].get_quantidade()//qtd), tropas[i].get_nivel() ))
for i in range(100):
    print('\n')
for i in range(2):
    ...
    for j in range(n):
        if(j%4 == 0):
            print(tropas_ordenadas_por_nivel[j].get_nivel(), ' Nível',end='\n')
        print('->  ', tropas_ordenadas_por_nivel[j].get_tipo(), ' nível ', tropas_ordenadas_por_nivel[j].get_nivel(), ' = ', tropas_ordenadas_por_nivel[j].get_quantidade(), sep='', end='\n')           
